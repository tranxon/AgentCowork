...
原有设计（[07-system-agent.md](./07-system-agent.md)）将用户身份管理委托给系统 Agent（`com.rollball.system`），通过 `identity_deps` → `identity_delivery` → `identity_store`/`identity_query` 工具的全链路实现身份管理。该方案引入了过多复杂性：系统 Agent 需要运行、需要 ContentProvider 机制、需要专用的内置工具、需要跨 Agent Intent 通信链路。
...
| `.opencode/style-guide.md` | 移除 identity 工具相关引用 |
...
| `docs/design/07-system-agent.md` | 添加废弃声明，引用本文档 |
