...
#### ACowork 不实现 RAG，只定义标准查询协议

ACowork 定义标准 HTTP 查询协议（请求/响应格式），企业 RAG 自行适配此协议：
...
ACowork 不为各家 RAG 实现 adapter，而是企业侧确保其 RAG 服务兼容此协议。这遵循 PRD "纯对接，不托管" 原则。
...
| D10 | RAG 协议适配方向 | **ACowork 定义标准协议，企业 RAG 自适配** | 不为各家 RAG 实现 adapter，企业侧确保其服务兼容标准查询接口；遵循 PRD "纯对接，不托管"原则 |
...
| RAG-02 标准查询接口 P2 | S4.2 | 完整实现，ACowork 定义协议，企业 RAG 自适配 |
...
| RAG 服务接口不兼容 | 中 | S4 无法对接 | ACowork 定义标准查询协议（请求/响应 JSON Schema），企业 RAG 自行适配此协议；提供接入指南和示例；不实现各家 RAG 的 adapter |
