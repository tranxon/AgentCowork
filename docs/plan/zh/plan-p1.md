...
ACowork 开发中优先复用已验证的模式和代码，避免重复造轮子。

**复用原则**：
- 只复用"相对独立"的代码——完整模块、完整函数、边界清晰的 trait
- 需要大幅修改才能适配 ACowork 架构的代码，不直接复用
- 不复用与单进程模式深度耦合的部分（如 Runtime 内嵌的 HTTP Server）

**Phase 1 重点复用领域**：

| 领域 | 参考来源 | ACowork 落地位置 | 说明 |
|------|---------|-----------------|------|
...
- 显著偏离时注明原因：`// ACowork deviation: <reason>`
- 不直接依赖外部 crate，而是 adaptation 后复制到对应 crate

**不直接复用的部分**（需要重新设计）：
- 单一巨型配置文件 → ACowork 按 crate 拆分
- 单进程 Agent Loop → ACowork 需要 IPC 层
- 文件系统沙箱（bubblewrap 集成方式）→ ACowork Phase 3 重新实现
- 配置驱动机制 → ACowork 改用 manifest 声明驱动
