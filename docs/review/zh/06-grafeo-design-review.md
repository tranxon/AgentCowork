...
**核心问题**：ACowork 的 Grafeo 使用方式与 Grafeo 实际能力存在系统性偏差
...
以下能力已在 Grafeo 中实现，ACowork 在重新发明轮子：

### 2.1 向量搜索（HNSW）

| 维度 | ACowork 当前设计 | Grafeo 原生能力 |
|------|------------------|----------------|
...
**关键缺失**：`grafeo-memory` 文档明确提到 `topology_boost`——搜索结果按图连通性重新排序。这是 Grafeo 图数据库的独特优势，ACowork 完全未使用。

**建议**：引入 Grafeo 的 topology boost 作为 ACowork 检索排序的一个维度。
...
ACowork 的 `graph_expand` 是用 SQL 在关系表上模拟图遍历。Grafeo 的图遍历是**数据库原生操作**，有查询优化器支持。
...
ACowork 重新实现了 Grafeo 原生提供的持久化机制。

### 2.6 并行执行

| 维度 | ACowork 当前设计 | Grafeo 原身能力 |
|------|------------------|----------------|
...
## 3. grafeo-memory：与 ACowork 设计高度重叠

这是最需要认真对待的部分。Grafeo 官方提供了 `grafeo-memory`（AI 记忆层），其架构与 ACowork 的 GrafeoMemoryManager **惊人地相似**：
...
### 3.3 ACowork 与 grafeo-memory 的取舍建议

ACowork 面临两个选择：

**路径 A（深度集成 Grafeo）**：直接使用 grafeo-memory 作为记忆层，ACowork 只需做少量 wrapper 和扩展：
- 优点：免费获得所有功能，复用成熟实现，Grafeo 团队维护
- 缺点：依赖 Python生态（grafeo-memory 是 Python 库），ACowork 是 Rust 项目

**路径 B（Rust-first 集成 Grafeo）**：用 Rust 调用 Grafeo 数据库（`grafeo-engine`），在 ACowork 的 MemoryManager 层重新实现记忆逻辑：
...
`grafeo-mcp` 将 Grafeo 数据库暴露给 AI Agent。如果 ACowork 用 Grafeo 作为存储，MCP 服务可以直接复用。

### 4.3 多查询语言

Grafeo 支持 GQL/Cypher 等多种查询语言。ACowork 的检索完全依赖 SQL-Like 操作，GQL 查询能力未使用。
...
| 7 | grafeo-memory 与 ACowork 设计高度重叠但未参考 | P1 | grafeo-memory 的 extract/reconcile 模式已成熟 |
| 8 | topology_boost 未使用 | P2 | Grafeo 独特优势，图连通性重排序 |
| 9 | MMR 多样性搜索未实现 | P2 | grafeo-memory 有，ACowork 无 |
| 10 | history() 变化历史 API 未实现 | P2 | grafeo-memory 有 |
| 11 | 图算法（PageRank/社区检测）未使用 | P2 | Grafeo 内嵌，ACowork 未提及 |
...
- Grafeo 的性能目标（1M 节点/秒插入）与 ACowork 的资源约束匹配
- Rust 原生集成，无生态依赖
...
- `grafeo-memory` 与 ACowork 记忆管理架构高度重叠，可参考设计思路
...
3. 参考 grafeo-memory 架构优化 ACowork MemoryManager 层
